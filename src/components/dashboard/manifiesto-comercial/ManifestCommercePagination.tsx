...
type ManifestCommercePaginationProps = {
    page: number
    totalPages: number
}

function ManifestCommercePagination( { page, totalPages } : ManifestCommercePaginationProps) {
        ...
        <nav className="flex justify-center py-10 gap-2 font-bold">
            {page > 1 && (
                <Link
                    ...
                    href={`/dashboard/manifiesto-comercial?page=${page - 1}`}
                >
                    &laquo;
                </Link>
            )}
            ...
            {pages.length > 6 ? (
                ...
                {pages.slice(0, 3).map((currentPage) => (
                    <Link
                        ...
                        href={`/dashboard/manifiesto-comercial?page=${currentPage}`}
                    >
                        {currentPage}
                    </Link>
                ))}
...
export default ManifestCommercePagination
