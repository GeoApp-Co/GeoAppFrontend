...
type ClientSelectInputProps = {
    setValue: UseFormSetValue<ManifestCommerceSearchFormData>
    watch: UseFormWatch<ManifestCommerceSearchFormData>
}

function ClientSelectInput( { setValue, watch } : ClientSelectInputProps) {
        ...
        />
            ...
            {selectedClient && (
                <div className="mt-4 border-2 shadow-xl rounded p-2 bg-gray-50 flex items-center border-azul justify-between">
                    <div>
                        <h3 className="font-semibold text-lg text-gray-700">Cliente seleccionado</h3>
                        <p>
                            <span className="text-azul font-semibold">Nombre:</span> {selectedClient.name}
                        </p>
                        <p>
                            <span className="text-azul font-semibold">Alias:</span> {selectedClient.alias}
                        </p>
                        <p>
                            <span className="text-azul font-semibold">Identificación ({selectedClient.identificacionType})</span>:{selectedClient.identificacion}
                        </p>
                    </div>
                    <button
                        type="button"
