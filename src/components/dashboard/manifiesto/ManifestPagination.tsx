...
type ManifestPaginationProps = {
    page: number
    totalPages: number
}

function ManifestPagination( { page, totalPages } : ManifestPaginationProps) {
        ...
        <nav className="flex justify-center py-10 gap-2 font-bold">
            {page > 1 && (
                <Link
                    ...
                    href={`/dashboard/manifiesto?page=${page - 1}`}
                >
                    &laquo;
                </Link>
            )}
            ...
            {pages.length > 6 ? (
                ...
                {pages.slice(0, 3).map((currentPage) => (
                    <Link
                        ...
                        href={`/dashboard/manifiesto?page=${currentPage}`}
                    >
                        {currentPage}
                    </Link>
                ))}
...
export default ManifestPagination
