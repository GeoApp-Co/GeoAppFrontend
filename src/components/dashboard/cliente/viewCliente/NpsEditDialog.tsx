"use client";
import { getNpsById, updateNps } from "@/src/api/npsApi";
...
export type NpsEditDialogProps = {
    open: boolean;
    onClose: () => void;
    npsId: string | null
    onRefresh?: () => void;
...
export default function NpsEditDialog({ open, onClose, npsId, onRefresh, clienteId}: NpsEditDialogProps) {
    ...
    const { data, isLoading, refetch, isError } = useQuery({
        queryKey: ['nps', npsId],
        queryFn: () => getNpsById({ clienteId: clienteId, id: Number(npsId) }),
        enabled: !!npsId && open && !!clienteId,
    })
    ...
    const onSubmit = (formData: NpsFormType) => {
        mutate({ clienteId, formData, id: Number(npsId)});
    };
    ...
    useEffect(() => {
        if (data) { 
            reset({
                ...
                personaEncuestada: data.personaEncuestada,
                cargo: data.cargo,
                encuestado: data.encuestado,
                // Adaptar formato de fecha de PostgreSQL (ISO string) a 'YYYY-MM-DD'
                fechaEncuesta: data.fechaEncuesta ? data.fechaEncuesta.slice(0, 10) : "",
